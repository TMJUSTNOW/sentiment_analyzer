...
with open('/home/john/sentiment_files/data/rotten_movie_vocab.pkl', 'rb') as f:
    data = pickle.load(f)
...
print(imdb_vocab_frequency[:100])
...
print(twitter_vocab_frequency[:100])

complete_vocab = [(val,freq) for val,freq in rotten_vocab_frequency_tuple[:9000] if val not in stop_words]
word_list = list(list(zip(*complete_vocab))[0])

for val, freq in imdb_vocab_frequency[:15000]+twitter_vocab_frequency[:15000]:
...
with open('/home/john/sentiment_files/data/complete_vocab_15_word.pkl', 'wb') as f:
